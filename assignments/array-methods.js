...
let fullNames = [];
runners.forEach(items => fullNames.push(`${items.first_name} ${items.last_name}`));
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = [];
runners.map(items => firstNamesAllCaps.push(`${items.first_name.toUpperCase()}`));
...
let ticketPriceTotal = runners.reduce((total, num) => total + num.donation, 0);
...
//The client wants the the runners categorized in groups based on their companies in alphabetical order.

let companyOrderedRunners = runners.sort((item1, item2) => (item1.company_name > item2.company_name)? 1 : -1);
console.log(companyOrderedRunners);

// Problem 2

//Client wants to single out the company Gigashots to confirm the info of runners.

let gigaOnly = runners.filter(object => object.company_name === "Gigashots")
console.log(gigaOnly);

// Problem 3

//An anonymous donor wants to donate an additional $100 to all donors who had donations of $15 or less. For documentation purposes, create a two new arrays with the runners who donated $15 or less and a another array with their new donation amounts after the addition of the anonymous donor's contribution.

let smallDonations = runners.filter(object => object.donation <= 15)
let newDonation = smallDonations.map(object => object.donation + 100);
